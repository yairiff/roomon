...
2. Copy `.env.example` to `.env` and set your Google client ID:
   ...
   cp .env.example .env
   ```
3. Update the allowed student emails in `src/data/allowedStudents.ts`.
4. Update rooms and labels in `src/data/rooms.ts`.
5. Update the schedule CSV in `src/data/rimon_schedule.csv`.
6. Start the app:
   ```bash
   npm run dev
   ```
...
The CSV is parsed via `src/lib/scheduleBuilder.ts` using column names defined in `src/data/rimonScheduleConfig.ts`.
